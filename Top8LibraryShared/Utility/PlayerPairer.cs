...
namespace Top8Library.Utility
{
    internal static class PlayerPairer
    {
        ...
        private static Stack<Tuple<IBasePlayer, IBasePlayer>> Pair2(IReadOnlyCollection<IBasePlayer> players)
        {
            var pairedPlayers = new List<Tuple<IBasePlayer, IBasePlayer>>((players.Count + 1) >> 1);
            var badMatchups = new Dictionary<IBasePlayer, HashSet<IBasePlayer>>();
            bool forcePairing = false;
            var sort = PlayerSorter.GetSorter(players.Count, "stable");
            var playerPool = players.ToList();
            //playerPool.Randomize();
            RandomData.Instance.Randomize(playerPool);
            sort(playerPool);

            if ((playerPool.Count & 1) == 1)//Unwind currently needs an even number to work.
            {
                playerPool.Add(new SimulatedByePlayer());
            }
            ...
            while (playerPool.Count > 0)
            {
                IBasePlayer playerA = playerPool[0];
                ...
                for (int i = 0; i < playerPool.Count; ++i)
                {
                    if (NotPlayed(playerA, playerPool[i], badMatchups))
                    {
                        playerB = playerPool[i];
